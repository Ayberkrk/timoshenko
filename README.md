...
Release **2.0.0** is an audit release with behavior changes. Observed modes are paired with reference modes by nearest frequency instead of by position. `review_recommended` is raised only when the caller passes `review_threshold_pct`, and never for changes within the spectral resolution. Single-channel damping comes from an averaged spectrum and is `None` when the record cannot resolve it. `load_sensors` rejects blank samples, SensorThings refuses redirects to another origin, and `MonitoringSession` validates options up front, persists a batch only after processing it, and reports model updates relative to the original model. The package now ships a regression test suite.
